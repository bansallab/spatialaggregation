...
weeksTemplate <- ctyDat %>%
  ...
  dplyr::mutate(uqid = paste(season, fips_st, onsetwk_cty, by = "_"))
cty_onset_1 <- ctyDat %>%
...
cty_onset <- cty_onset_1 %>%
  ...
  dplyr::arrange(fips_st, onsetwk_cty) %>% 
...
## dummy fill in cumpop_prop_cty as 0 for weeks before season onset
inRealData <- cty_onset %>%
  distinct(onsetwk_cty, fips_st) %>%
  dplyr::mutate(uqid = paste(onsetwk_cty, fips_st, by = "_"))
realOnset <- cty_onset %>%
  group_by(fips_st, season) %>%
  dplyr::filter(onsetwk_cty == min(onsetwk_cty)) %>%
  ungroup %>%
  dplyr::select(fips_st, season, onsetwk_cty) %>%
  dplyr::rename(realOnsetWk = onsetwk_cty)
...
st_onset <- stDat %>%
  ...
  dplyr::select(fips_st, season, onsetwk_st) 
...
reg_onset <- regDat %>% 
  ...
  dplyr::filter(Thu.week == min(Thu.week)) %>%
  ungroup %>%
  dplyr::rename(onsetwk_reg = Thu.week) %>%
  dplyr::select(region, season, onsetwk_reg)
...
onsetDat <- bind_rows(cty_onset, dummyStartEnd_st) %>%
  dplyr::arrange(fips_st, onsetwk_cty) %>%
  full_join(st_onset, by = c("fips_st", "season")) %>%
  full_join(reg_onset, by = c("region", "season")) %>%
  left_join(state_colors, by = c("fips_st")) %>%
  dplyr::mutate(plt.date_cty = ifelse(lubridate::month(onsetwk_cty)>=11, paste0("2005", substring(onsetwk_cty, 5, 10)), paste0("2006", substring(onsetwk_cty, 5, 10)))) %>%
  dplyr::mutate(plt.date_cty = lubridate::as_date(plt.date_cty)) %>%
  dplyr::mutate(plt.date_st = ifelse(lubridate::month(onsetwk_st)>=11, paste0("2005", substring(onsetwk_st, 5, 10)), paste0("2006", substring(onsetwk_st, 5, 10)))) %>%
...
plt <- ggplot(onsetDat, aes(x = plt.date_cty, y = cumpop_prop_cty, group = fips_st)) +
  ...
  scale_x_date("Onset Week", date_labels = "%b", date_breaks = "2 months") +
...
ggsave("graph_outputs/explore_irDt_cumpop/onset_cumpop_region_season_grid.png", plt, width = 7, height = 9)

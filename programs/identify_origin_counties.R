...
wksToEpiDat <- read_csv("R_export/inlaModelData_import/inlaImport_model8f_wksToEpi_irDt_v7.csv")
...
srcLocDat_corr <- fullDat %>% 
  group_by(season) %>% 
  dplyr::top_n(50, corrCoef) %>% 
  ungroup %>%
  left_join(earlyLocDat %>% distinct(srcFips, fips_st, srcLat, srcLon), by = c("srcFips")) %>% 
  dplyr::select(season, srcFips, srcLat, srcLon, corrCoef)

write_csv(srcLocDat_corr, "R_export/origin_locations/fluseason_source_counties.csv")
